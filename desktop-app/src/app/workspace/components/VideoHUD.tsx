...
import { Expand, FastForward, Pause, Play, Radio, SkipBack, Video } from "lucide-react";

import { type JobArtifactsResponse, getApiBaseUrl } from "@/lib/api";
...
export function VideoHUD({ jobId, artifacts }: VideoHUDProps): React.JSX.Element {
  ...
  const overlayUrl = useMemo(() => {
    ...
    return `${getApiBaseUrl()}/api/v1/jobs/${jobId}/overlay`;
  }, [jobId]);
    ...
    <div className="w-full max-w-6xl h-[280px] flex gap-4 items-center justify-center font-mono animate-fade-in z-20">
      ...
      <div className="flex-1 h-full relative bg-[#0a0f0a] rounded-xl overflow-hidden border border-gray-800 shadow-[0_0_30px_rgba(0,0,0,0.5)] flex flex-col group">
        ...
        {overlayUrl && (
          <video
            src={overlayUrl}
            className="absolute inset-0 h-full w-full object-cover z-[1] opacity-80"
            controls={false}
            autoPlay
            muted
            loop
           ...
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 text-gray-300">
                 ...
                 <span className="text-xs font-mono ml-2">
                   {artifacts?.status === "done" ? "Overlay Ready" : "Waiting for overlay"}
                 </span>
              </div>
