...
Protocol:
    ...
    3. POST /api/v1/upload/complete   → reassemble + create pipeline job
    4. GET  /api/v1/upload/{id}/status → check which chunks arrived (for resume)

Design:
    - Chunks are written directly to disk with a 64 KB buffer (no RAM buffering).
...
#              quality_profile, chunking_interval) -> None
JobCreatorFn = Callable[
    [str, Path, str, str, str, str, str],
    Awaitable[None],
]
...
# Resolved once at import; overridden in ``configure_upload_dir()``.
_UPLOAD_ROOT: Path = Path(__file__).resolve().parent.parent / "data" / "uploads"


def configure_upload_dir(root: Path) -> None:
    """Allow ``main.py`` to set the uploads root at startup."""
    global _UPLOAD_ROOT
    _UPLOAD_ROOT = root
...
async def init_upload(
    ...
    LOGGER.info(
        "Upload session %s created: %s (%d bytes, %d chunks)",
        upload_id[:8],
        filename,
        total_size,
        total_chunks,
...
async def complete_upload(
    ...
    try:
        ...
        LOGGER.info(
            "Reassembled %s → %s (%d bytes on disk)",
            upload_id[:8],
            final_path.name,
